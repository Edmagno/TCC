...
2.  **Execução em Lote:** Cada script executou os algoritmos (da pasta `/Algoritmos`) aplicando os vetores gerados (da pasta `/Vetores`).
3.  **Coleta Estatística:**  A execução consistiu consistiu em **ordenar os 100 vetores diferentes** de cada tamanho, um de cada vez.
4.  **Medição:** O tempo de execução de *cada uma* dessas 100 ordenações foi cronometrado individualmente.
...
6.  **Armazenamento:** Esses arquivos `.csv` foram salvos diretamente na pasta `/Resultados`.
...
4.  **Análise:** Com os dados tratados e agregados, são gerados os gráficos comparativos (usando Matplotlib e Seaborn) para responder às perguntas do TCC.
